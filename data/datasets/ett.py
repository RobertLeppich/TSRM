...
class ETTm2Dataset(AbstractDataset):
    ...
    def split_into_slices(self, dataset, sice, data_slice: float):
        result = []
        i = 0
        while True:
    ...
    def prepare(self):
        dataset = pk.load(open("data_dir/ettm2.pk", "rb"))
        self.scaler = dataset["scaler"]

        ds = dataset["dataset"]

        train_size, validation_size, test_size = self.config["validation_size"] + self.config["test_size"], self.config[
            "validation_size"], self.config["test_size"]
        train_size, validation_size, test_size = int(train_size * len(ds)), int(validation_size * len(ds)), int(
            test_size * len(ds))
        ...
        if self.config["feature_dimension"] == 1:
            train_data = train_data.loc[:, ["OT"]]
            validation_data = validation_data.loc[:, ["OT"]]
            test_data = test_data.loc[:, ["OT"]]
